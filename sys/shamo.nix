...
in rec {
  ...
  services.vector = {
    ...
    settings = {
      ...
      sinks.cw = {
        ...
        auth.credentials_file = "/run/credentials/vector/cloudwatch"; # I'd prefer to use "\${CREDENTIALS_DIRECTORY}/cloudwatch", but that fails the verify check
  ...
  systemd.services.vector.serviceConfig.LoadCredential = "cloudwatch:/etc/secrets/cloudwatch";
